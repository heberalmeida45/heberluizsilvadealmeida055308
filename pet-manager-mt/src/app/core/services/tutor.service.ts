...
export class TutorService {
  ...
  // Listagem com paginação (Essencial para a lista de Várzea Grande)
  listarTutores(page: number = 0, size: number = 10): Observable<any> { 
    return this.http.get(`${this.apiUrl}?page=${page}&size=${size}`);
  }

  // Busca por ID (Unificado para evitar erros de compilação)
  ...
  // Gestão de Fotos (Integração com a API GEIA)
